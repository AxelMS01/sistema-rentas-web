...
import { Eraser, Grid2X2Check, TriangleAlert } from "lucide-react";
...
export default function WelcomeForm({ firstName }) {
    ...
    async function onSubmitData(e) {
        ...
        if (!guarantorConfirmed) {
            toast.error("Confirma primero los datos del aval para continuar.");
            return;
        }
    ...
    async function handleFinishForm() {
        if (!tenantSignatureRef.current || !guarantorSignatureRef.current) {
            toast.error("¡Las firmas no pueden quedar vacías!");
            return;
        }

        const tenantSignatureURL = tenantSignatureRef.current.canvas.toDataURL();
        const guarantorSignatureURL = guarantorSignatureRef.current.canvas.toDataURL();

        const { error: tenantError } = await supabase
            .from("tenants")
            .update({
                is_first_time: false,
                signature_url: tenantSignatureURL,
            })
            .eq("id", location.id);
        ...
        const { error: guarantorError } = await supabase
            .from("guarantors")
            .update({
                signature_url: guarantorSignatureURL,
            })
            .eq("id", contractInfo.guarantorid);

        const guarantorSignatureColumnMissing =
            guarantorError?.code === "PGRST204" &&
            guarantorError?.message?.includes("signature_url");

        if (guarantorError && !guarantorSignatureColumnMissing) {
        ...
        if (guarantorSignatureColumnMissing) {
            toast("La firma del aval no se guardó todavía porque falta la columna signature_url en la tabla guarantors.");
        }
    ...
    async function handleIncorrectData() {
        ...
        navigate("/home", {
            state: { welcomeFormErr: "Le notificaremos al arrendador que hubo un error. ¡Gracias!" },
        });
    }
        ...
        <>
            {!isDataLoading && (
                <div className="w-full min-h-screen h-auto flex flex-col items-center justify-center gap-4! lg:px-20! sm:px-16! px-8! py-10 bg-sky-600">
                    ...
                    <IncorrectDataModal
                        isModalOpen={incorrectDataModal}
                        onCloseModal={() => setIncorrectDataModal(false)}
                        ownerId={ownerInfo.id}
                    ...
                    <div className="form-content flex flex-col max-w-xl w-auto bg-white rounded-xl gap-8">
                        <div className="welcome-message flex flex-col gap-1 items-start p-8 border-b border-b-slate-200">
                            ...
                            <p className="text-slate-600 text-start">
                                Para empezar a usar el sistema, por favor, completa el siguiente formulario para terminar de generar tu contrato.
                            </p>
                        </div>
                        ...
                        <form onSubmit={onSubmitData} className={`flex flex-col gap-4 sm:px-8 px-6 sm:pb-8 pb-6 pt-0 ${currentStep !== 3 ? "max-h-96" : "h-auto"} overflow-y-scroll`}>
                            ...
                            {currentStep === 2 && (
                                <StepTwoContent
                                    ...
                                    guarantorFatherSurname={guarantorInfo?.father_surname || ""}
                                    guarantorMotherSurname={guarantorInfo?.mother_surname || ""}
                                    guarantorConfirmed={guarantorConfirmed}
                                    setGuarantorConfirmed={setGuarantorConfirmed}
                                />
                            )}
                                ...
                                <>
                                    <div className="flex flex-col gap-4 items-start">
                                        ...
                                        <p className="text-base text-slate-600">
                                            La siguiente es una vista previa del contrato que será generado. Si estás de acuerdo con la información, por favor, pasa a firmarlo en el siguiente paso.
                                        </p>
                                    </div>
                            ...
                            {currentStep === 4 && (
                                <>
                                    ...
                                    <div className="flex flex-col gap-4 items-start justify-start w-full">
                                        <p className="text-start text-sm! text-slate-800">
                                            Por favor, dibuja la firma del arrendatario y la del aval. Ambas se usarán en la versión final del contrato.
                                        </p>

                                        <div className="flex flex-col gap-2 items-start w-full">
                                            <p className="text-sm! font-medium! text-slate-900">Firma del arrendatario</p>

                                            <div className="relative signature-container items-start border border-slate-200 rounded-xl bg-slate-50 w-full">
                                                <Button onClick={() => tenantSignatureRef.current.clear()} color="alternative" size="xs" className="absolute z-999 top-3 right-3 text-xs! font-semibold! px-2 py-0.5! rounded-lg! flex flex-row gap-2 items-center">
                                                    <Eraser size={16} />
                                                    Restablecer
                                                </Button>
                                        ...
                                        <div className="flex flex-col gap-2 items-start w-full">
                                            <p className="text-sm! font-medium! text-slate-900">Firma del aval</p>

                                            <div className="relative signature-container items-start border border-slate-200 rounded-xl bg-slate-50 w-full">
                                                <Button onClick={() => guarantorSignatureRef.current.clear()} color="alternative" size="xs" className="absolute z-999 top-3 right-3 text-xs! font-semibold! px-2 py-0.5! rounded-lg! flex flex-row gap-2 items-center">
                                                    <Eraser size={16} />
                                                    Restablecer
                                                </Button>
                        ...
                        <div className="px-8 pb-8 pt-0 flex flex-col gap-4">
                            <div className="flex sm:flex-row flex-col w-full items-center gap-2">
                                {currentStep > 1 && (
                                    <Button onClick={() => setCurrentStep((prevStep) => prevStep - 1)} className="text-sm! w-full rounded-md! py-0!" color="alternative">
                                        Regresar
                                    </Button>
                                )}

                                <Button
                                    disabled={currentStep === 4 && !signAuthorization}
                                    type="button"
                                    onClick={currentStep === 2 ? onSubmitData : currentStep === 4 ? handleFinishForm : () => setCurrentStep((prevStep) => prevStep + 1)}
                                    className="text-sm! w-full text-nowrap rounded-md! py-0! bg-sky-600 hover:bg-sky-700!"
                                    color="default"
                                >
