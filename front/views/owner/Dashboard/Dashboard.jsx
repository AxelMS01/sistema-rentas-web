...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetching data related to the maintenance requests.
      ...
      setDashboardData({ ...dashboardData, dataIncidencias: requestsData });
  ...
  const datosProcesados = useMemo(() => {
    ...
    if (dashboardData.dataBaseInquilinos) {
  ...
  if (dashboardData.dataIncidencias != undefined) {
    incidenciasVisibles = dashboardData.dataIncidencias.filter((request) => request.status != "pending");
  }
            ...
            <div className="card w-100 border-0 rounded-4 overflow-hidden" style={{ backgroundColor: THEME.bgCard }}>
              ...
              <div className="table-responsive px-2 pb-3">
                <table className="table w-100 table-borderless align-middle mb-0">
                  ...
                  <tbody>
                    {datosProcesados ? datosProcesados.map((item) => (
